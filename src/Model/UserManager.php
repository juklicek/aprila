...
class UserManager implements Nette\Security\IAuthenticator
{
    ...
    */
    ...
    {
        $password = $this->passwords->hash($form['password']);
    ...
    public function changeUserPassword($password, $new_password, $userId)
    {
        ...
        if (!$this->passwords->verify($password, $user->password)) {
            ...
            $this->database->table('users')
                ->where('id', $userId)
                ->update(array('password' => $this->passwords->hash($new_password)));
    ...
    public function setUserPassword($password, $userId)
    {
        ...
        if ($user) {
            $user->update(array('password' => $this->passwords->hash($password)));
                                       ...
                                       Nette\Mail\IMailer $mailer)
    {
        ...
        $tokenHashOne = $this->passwords->hash($tokenOne);
        $tokenHashTwo = $this->passwords->hash($tokenTwo);
    ...
    public function changeEmailStepTwo($id, $tokenOne, $tokenTwo)
    {
        ...
        if ($this->passwords->verify($tokenOne, $person->change_email_tokenOne)) {
            if ($this->passwords->verify($tokenTwo, $person->change_email_tokenTwo)) {
                $verified = TRUE;
            }
        } else {
            // swithed codes?
            if ($this->passwords->verify($tokenOne, $person->change_email_tokenTwo)) {
                if ($this->passwords->verify($tokenTwo, $person->change_email_tokenOne)) {
                    $verified = TRUE;
                }
            }
        }
